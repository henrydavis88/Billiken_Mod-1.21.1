...
public class BillikenMod {
    // Define mod id in a common place for everything to reference
    // Define mod id in a common place for everything to reference
    public static final String MOD_ID = "billikenmodneo";
    // Directly reference a slf4j logger
    public static final Logger LOGGER = LogUtils.getLogger();


    public BillikenMod(IEventBus modEventBus,  ModContainer modContainer) {
    ...
    private void addCreative(BuildCreativeModeTabContentsEvent event) {
        if(event.getTabKey() == CreativeModeTabs.TOOLS_AND_UTILITIES) {
            event.accept(ModItems.TUITION);
        }

        if(event.getTabKey() == CreativeModeTabs.BUILDING_BLOCKS) {
            event.accept(ModBlocks.TUITION_BLOCK);
            event.accept(ModBlocks.BILLIKEN_BLOCK);
        }
    }
